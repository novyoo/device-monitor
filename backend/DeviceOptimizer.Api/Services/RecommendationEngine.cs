namespace DeviceOptimizer.Api.Services
{
    ...
    public static class RecommendationEngine
    {
            ...
            double lifetimeActiveUseHours)
        {
            ...
            {
                reasons.Add($"Health score is {healthScore}% ({bandDisplay}) — in good shape to rent out again.");
                return new RecommendationResult { Action = "RentAgain", Reasons = reasons };
            }

            if (healthBand == "ActNow" && (repairCount > 0 || ageYears >= 2))
            {
                reasons.Add($"Health score is {healthScore}% ({bandDisplay}) — condition is too poor to keep in the fleet.");
                reasons.AddRange(healthReasons);
                return new RecommendationResult { Action = "Retire", Reasons = reasons };
            }

            reasons.Add($"Health score is {healthScore}% ({bandDisplay}) — worth repairing before renting out again.");
            reasons.AddRange(healthReasons);
